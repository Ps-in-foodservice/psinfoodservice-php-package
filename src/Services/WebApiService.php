...
use PSinfoodservice\Dtos\Outgoing\ProductSheetDto;
use PSinfoodservice\Dtos\Outgoing\RequestStatusItemDto;
...
class WebApiService
{
     ...
     * @return ProductSheetDto|null The product sheet data or null if not available
     ...
     *     echo "Product: " . $productSheet->getBrandName() . "\n";
     *     echo "GTIN: " . $productSheet->getGtin() . "\n";
     ...
     * ```
     */
    public function getProductSheet(int $logisticId, string $language = Language::all): ?ProductSheetDto
    {
        try {
            ...
            return ProductSheetDto::fromData($data);
     ...
     *
     * @return RequestStatusItemDto[]|null Array of user's products or null if no products are available
    ...
    public function getMyProducts(): ?array
    {
        try {
            ...
            return array_map(
                fn($item) => RequestStatusItemDto::fromData($item),
                $data->Items
            );
        } catch (ClientException $e) {
            $errorResponse = json_decode($e->getResponse()->getBody()->getContents(), true);
